
class AppConstants{
  static const String APP_NAME = "pancakes";// name of app
  static const int APP_VERSION = 1;

  static const String BASE_URL = "http://asdf-env.eba-eemi8fuw.us-east-2.elasticbeanstalk.com";

  ///user endpoints
  static const String REGISTER_URI = "/api/register";
  static const String LOGIN_URI = "/api/login";
  ///user endpoints
  ///
  ///
  /// Get data endpoints
  static const String GET_CURRENT_STATE = "/api/get-current-state";
  static const String GET_PLANT_LIST = "/api/get-plant-list";
  /// Get data endpoints

  static const String NAME = "name";
  static const String USER_ID = "user_id";//TODO: change tohi to uuid
  static const String TOKEN = "token";
  static const String HAS_LINKED_ACCOUNT = "false";

}